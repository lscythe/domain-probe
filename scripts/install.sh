...
repo="${REPO:-lscythe/domain-checker}"
...
archive="domain-checker_${version#v}_${os}_${arch}.tar.gz"
tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT
curl -fsSL "https://github.com/$repo/releases/download/$version/$archive" -o "$tmp/$archive"
tar -xzf "$tmp/$archive" -C "$tmp"
mkdir -p "$install_dir"
install "$tmp/domain-checker" "$install_dir/domain-checker"
printf 'installed domain-checker %s to %s\n' "$version" "$install_dir/domain-checker"
